...
(function() {
  ...
  const C_KEYWORDS = [FLOW_CONTROL_KEYWORDS,
    "auto,case,char,const,default," +
    "double,enum,extern,float,goto,int,long,register,short,signed,sizeof," +
    "static,struct,switch,typedef,union,unsigned,void,volatile"
  ];
  const COMMON_KEYWORDS = [C_KEYWORDS,
    "catch,class,delete,false,import," +
    "new,operator,private,protected,public,this,throw,true,try,typeof"
  ];
  const CPP_KEYWORDS = [COMMON_KEYWORDS,
  ...
  const PYTHON_KEYWORDS = [FLOW_CONTROL_KEYWORDS,
    "and,as,assert,class,def,del," +
    "elif,except,exec,finally,from,global,import,in,is,lambda," +
    "nonlocal,not,or,pass,print,raise,try,with,yield," +
    "False,True,None"
  ];
  const RUBY_KEYWORDS = [FLOW_CONTROL_KEYWORDS,
    "alias,and,begin,case,class," +
    "def,defined,elsif,end,ensure,false,in,module,next,nil,not,or,redo," +
    "rescue,retry,self,super,then,true,undef,unless,until,when,yield," +
    "BEGIN,END"
  ];
  const SH_KEYWORDS = [FLOW_CONTROL_KEYWORDS,
  ...
  function combinePrefixPatterns(regexs) {
    ...
    function caseFoldCharset(charSet) {
      const charsetParts = charSet.substring(1, charSet.length - 1).match(new RegExp(
        '\\\\u[0-9A-Fa-f]{4}' +
        '|\\\\x[0-9A-Fa-f]{2}' +
        '|\\\\[0-3][0-7]{0,2}' +
        '|\\\\[0-7]{1,2}' +
    ...
    function allowAnywhereFoldCaseAndRenumberGroups(regex) {
      ...
      const parts = regex.source.match(new RegExp(
        '(?:' +
      ...
      // mapping.
      for (var groupIndex = 0, i = 0; i < n; ++i) {
        var p = parts[i];
        if (p === '(') {
  ...
  function extractSourceSpans(node) {
    ...
    const isPreformatted = whitespace && whitespace.substring(0, 3) === 'pre';
  ...
  function sourceDecorator(options) {
    ...
    if (options.regexLiterals) {
      ...
      const REGEX_LITERAL =
        ...
        // finally closed by a /.
        '/';
      fallthroughStylePatterns.push(['lang-regex',
    ...
    if (types) {
      fallthroughStylePatterns.push([PR_TYPE,
        types
      ]);
    }
    ...
    shortcutStylePatterns.push([PR_PLAIN,
      /^\s+/,
      null,
      ' \r\n\t\xA0'
    ]);
  ...
  function recombineTagsAndDecorations(job) {
    const isIE = (/\bMSIE\b/).test(navigator.userAgent);
    const newlineRe = /\n/g;
    ...
    while (spanIndex < nSpans) {
      ...
      const decStart = decorations[decorationIndex];
      ...
      if (sourceIndex >= decEnd) {
        decorationIndex = decorationIndex + 2;
      }
    }
  }
  ...
  function applyDecorator(job) {
    ...
    try {
      ...
      const sourceAndSpans = extractSourceSpans(job.sourceNode);
  ...
  function prettyPrint(opt_whenDone) {
    ...
    let codeSegments = [byTagName('pre'),
      byTagName('code'),
      byTagName('xmp')
    ];
    const elements = [];
    ...
    function doWork() {
      const endTime = window.PR_SHOULD_USE_CONTINUATION ?
        clock.now() + 250 :
        /* ms */ Infinity;
      for (; k < elements.length && clock.now() < endTime; k++) {
        ...
        if (className.indexOf('prettyprint') >= 0) {
          ...
          let langExtension = className.match(langExtensionRe);
          ...
          if (!nested) {
            ...
            let lineNums = cs.className.match(/\blinenums\b(?::(\d+))?/);
            lineNums = lineNums ?
